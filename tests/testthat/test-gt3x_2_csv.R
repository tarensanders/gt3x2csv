...
test_that("convert_file() works if the last row is missing", {
  skip_on_ci()
  skip_on_cran()
  skip_if(!file.exists(test_path("large_tests", "large_missing.gt3x")))
  ...
  # Process the same file in gt3x2csv
  actilife_ver <- "ActiLife v6.11.9"
  gt3x_file <- test_path("large_tests", "large_missing.gt3x")

  outfile <- file.path(dir, "test_large_missing.csv")
